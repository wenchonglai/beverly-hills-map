    ...
    this.generateBldg=function(DGL,which){
        ...
        if (floorTemp[0]){
            ...
            if (floorTemp[1]){
                ...
                $("#Area_Actual").html((floor[0].area()+floor[1].area())+" sf");
            ...
            else if ((floor[0].area()+floor[1].area())/DGL.area.total(which.shape.area())<0.80){
                $("#Error_Log").css("color","#ff0000");
                $("#Error_Log").html("The given zoning regulations are already severely limiting buildable area!")
            }
            else if ((floor[0].area()+floor[1].area())/DGL.area.total(which.shape.area())<0.95){
    ...
    this.load=function(poly,DGL){
        ...
        if (!(poly instanceof Poly)) return false   
            ...
            var determineLotWidth=function(which){
                ...
                p1.setPath(temp[0]);p2.setPath(temp[temp.length-1]);p3.setPath(temp2[0]);p4.setPath(temp2[temp2.length-1]);
